...
public class TeamCommand implements CommandExecutor {
	...
	public TeamCommand(final GameManager gameManager) {
	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (args[0].equalsIgnoreCase("leave")) {
			...
			final UHCTeam team = gameManager.getTeamManager().getTeam(args[0]);
			...
			final UHCTeam team = gameManager.getTeamManager().getTeam(args[1]);
			team.addMember(uhcPlayer);
		} else {
			new TeamsMenu(MenuManager.getMenuUtils(uhcPlayer)).open();
		}
